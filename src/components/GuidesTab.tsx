...
const GUIDES: GuideSection[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {
        id: "quick-deliver-single",
        title: "Quick Deliver — Single Item Order",
        content: "This is your **most common daily action**. Here's the step-by-step:\n\n1. Find the pending order in the list\n2. Click the **⚡ Deliver** button on the order row\n3. The order card expands to show the delivery form\n4. Fill in **Username / Email** (required)\n5. Fill in **Password** (required)\n6. Optionally add: TOTP Secret, Notes\n7. **Expiry Date** is auto-calculated from the product duration (editable if needed)\n8. Click **✓ Deliver & Complete**\n\nWhat happens behind the scenes:\n• A credential record is created and linked to this order\n• The order status changes to \"completed\"\n• The customer receives a push notification: \"✅ Your order is ready!\"\n• The credential becomes visible on the customer's /lookup page",
        tips: ["Expiry date is auto-filled based on the product duration (e.g., 1 Month = 30 days from today).", "You can edit the expiry date if you need a custom duration.", "The customer gets notified automatically — no need to message them manually."],
        goTo: "orders",
        goToLabel: "Go to Orders",
      },
      {
        id: "quick-deliver-multi",
        title: "Quick Deliver — Multi-Item Orders",
        content: "When a customer orders multiple products (e.g., ChatGPT Pro + Netflix 4K in one cart), the Quick Deliver form currently handles the **first item** in the order.\n\nFor additional items:\n1. Deliver the first item using Quick Deliver as normal\n2. Go to the **Credentials** tab\n3. Click **+ Add Credential**\n4. Enter the same **Order Code** and **Phone** number\n5. Set the correct product name and duration for the second item\n6. Fill in the credentials and save\n\nThis links both credentials to the same order, and the customer sees all of them on their lookup page.",
        warnings: ["Currently, Quick Deliver only creates one credential per order. For multi-item orders, create additional credentials manually."],
        goTo: "credentials",
  ...
  },
  {
    ...
    steps: [
      {
        id: "auto-created",
        title: "Auto-Created via Quick Deliver",
        content: "When you Quick Deliver an order, a credential record is **automatically created** with:\n• Order code and phone number (from the order)\n• Product name and duration (from the first order item)\n• Username and password (from your delivery form)\n• Start date (today) and expiry date (calculated from duration)\n• Optional: TOTP secret and notes\n\nYou don't need to do anything extra — it's all handled in one click.",
        goTo: "credentials",
        goToLabel: "Go to Credentials",
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        content: "After you deliver credentials, the customer retrieves them at `/lookup`:\n\n1. Customer enters their phone number\n2. Receives a TOTP code (time-based one-time password)\n3. Enters the code to verify identity\n4. Sees all their credentials:\n   • Product name and duration\n   • Username / email\n   • Password (hidden by default, revealable)\n   • Expiry date with countdown\n   • TOTP code generator (if applicable)\n\nThis page is **device-bound** — the first device that accesses it becomes the authorized device for that phone number.",
      },
      {
